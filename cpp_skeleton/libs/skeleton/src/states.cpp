...
StatePtr RoundState::showdown() const {
  return std::make_shared<TerminalState>(
      std::array<int, 2>{0, 0}, std::array<bool, 2>{false, false}, getShared());
}
...
std::unordered_set<Action::Type> RoundState::legalActions() const {
  ...
  auto raisesForbidden =
      continueCost == stacks[active] || stacks[1 - active] == 0;
...
StatePtr RoundState::proceedStreet() const {
  ...
  return std::make_shared<RoundState>(1, newStreet, std::array<int, 2>{0, 0},
                                      stacks, hands, bounties, deck,
                                      getShared());
}
...
StatePtr RoundState::proceed(Action action) const {
  ...
  switch (action.actionType) {
    case Action::Type::FOLD: {
      auto delta =
          active == 0 ? stacks[0] - STARTING_STACK : STARTING_STACK - stacks[1];
      return std::make_shared<TerminalState>(
          std::array<int, 2>{delta, -1 * delta}, get_bounty_hits(),
          getShared());
    }
    case Action::Type::CALL: {
      ...
      auto state = std::make_shared<RoundState>(
          button + 1, street, std::move(newPips), std::move(newStacks), hands,
          bounties, deck, getShared());
    ...
    case Action::Type::CHECK: {
      ...
      // let opponent act
      return std::make_shared<RoundState>(button + 1, street, pips, stacks,
                                          hands, bounties, deck, getShared());
    }
...
std::array<bool, 2> RoundState::get_bounty_hits() const {
  ...
  */
  ...
  try {
    for (int i = 0; i < 2; i++) cards0.push_back(this->hands[0][i][0]);
  } catch (const std::exception &e) {
  }
  try {
    for (int i = 0; i < 2; i++) cards1.push_back(this->hands[1][i][0]);
  } catch (const std::exception &e) {
  }
  ...
  std::array<bool, 2> bounty_hits = {
      std::find(cards0.begin(), cards0.end(), this->bounties[0]) !=
          cards0.end(),
      std::find(cards1.begin(), cards1.end(), this->bounties[1]) !=
          cards1.end()};
  return bounty_hits;
}
